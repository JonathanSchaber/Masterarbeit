...
There are a few remarks and strategi decisions that apply to all collected dataset in GerGLUE:
...
ratio. The only dataset having explicitely predefined training, development, and test splits
is PAWS-X.

(3) Several of thedata sets were constructud by translating existing monolingual English sources
(semi-)automatically into the different target languages. As I show in chapter \ref{chap:results},
...
Traurigkeit, Ekel, Schuld, Wut, Angst, Scham, Freude.\myfootnote{Interestingly, out of these
seven emotions, six represent rather negative emotions --- only \emph{Freude} is a clearly positive sensation.
Maybe negative emotions are more lucidly detactable (by humans and/or machines) than positive ones, I leave
this question for the reader to further explore.}
...
not reported. The presence of sometimes strange offsets (included commas, misssing
...
identify the correct answer\myfootnote{\rotatebox{180}{überwiegend antibritisch
und prorussisch}} span for the following context-question pair (to be ``fair'' and have the same conditions
...
Concretely, \citeauthor{yang2019paws} ``translate the Wikipedia portion of the original
PAWS corpus from English'' to six target languages. Out of these, I include the German
part into GerGLUE. These corpora were created by hiring human translators to translate
the original PAWS development and test sets into the six languages, while for the training
set a ``neural machine translation'' system was employed.
...
The gold labels for the following examples you are invited to try to answer yourself are in this
footnote\myfootnote{\rotatebox{180}{\ref{itm:paws-x1}: False, \ref{itm:paws-x2}: True}}.
...
empty strings; I treat those the same way as the examples mentioned before.
...
($\sigma$ 8.6), 27.7 ($\sigma$ 8.4), and 28.1 ($\sigma$ 8.4) for subtokenized training,
development, and test sets. Also the lenght differences between the first and second
sentence in each example is virtually inexistent, which is not surprising if the creation
...
Solutions\myfootnote{\rotatebox{180}{\ref{itm:scare1}: Negative, \ref{itm:scare2}: Positive, \ref{itm:scare3}: Positive}}
...
reveals that the positive class amasses the most exmples on itself (1,071), while the neutral class only comprises
193 items out of a total of 1,760 examples (cf. figure \ref{fig:scare-stats}). However, as the analysis of the
GliBERT predictions in chapter \ref{chap:results} reveals, the does not simply overweigh the majority class.
...
i.e. that the title and the text of the review form on continuous sequence. Nevertheless, the
...
Here are the answers\myfootnote{\rotatebox{180}{\ref{itm:xnli1}: Neutral, \ref{itm:xnli2} Entailment, \ref{itm:xnli3} Neutral}}
...
that consits in re-training a monolingual English BERT model on Wikipedia and transfer it to the
...
the splits and nature of the splits (predifined or not), the style or register of the text
present in the dataset, as well as some important distinctiveness of each dataset. Note that
the ordering is not strictly alphabetical, but follows the folling grouping: First, come the
