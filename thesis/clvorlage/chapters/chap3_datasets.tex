...
identify the correct answer\myfootnote{\rotatebox{180}{\textbf{Answer}: überwiegend antibritisch
...
examples. I treat the MLQA development set as training set and split off a 15\% portion of
it as development set.

As can bee seen in the left figure of \ref{fig:mlqa-stats}, MLQA comprises quite a high number ---
868, to be precise ---  of examples which do not fit subtokenized into a normal BERT model. However,
those examples where the answer span lies inside the maximum BERT sequence are simply cut off after
the maximum length and are kept in the data set for the experiments. Only the 22 exmaples where the
answer span lies partially or fully outside the maximum length were dropped.
