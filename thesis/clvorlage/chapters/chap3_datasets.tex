...
exaples where the answer span lies partially or fully outside the maximum length were dropped.
...
is typically formalized as a binary classifiaction task: given two sentences $S_1$ and
...
You are invited to test yourself and detext if the following sentence pairs are in fact rephrasings
...
To generate the review-label, I simply carry out a majority class decision:
The polarity that is most often annotated for a given review is also the review-label; in the
example above comprisign three aspect-subjective polarities, two of them positive, one negative,
...
Follwoing are three examples to test for yourself with the gold labels in this
...
GliBERT predictions in chapter \ref{chap:results} discloses, the models do not simply overweigh the majority class.
...
created the hypothesis and the labels that were assigend indepentently to the sentence pair by
four other people. Finally, all the sentence pairs were translated into the different languages by
tranlators. In addition, \cite{conneau2018xnli} carry out some tests to verify that the original
...
sentence pairs. For the English examples, \citeauthor{conneau2018xnli} find a 85\% concesus on the
