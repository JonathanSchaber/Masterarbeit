...
self-attention mechanism introced by \cite{vaswani2017attention}; the
main difference, and apparently advantage, to other architectures
that implement the transformer architecutre is that  BERT is a bi-directional
...
especially remarkink, since BERT is not a highly specialized model\myfootnote{Until then, SOTAs
...
Deep-Cases. This approach, known as ``Lexical Conceptual Structures'' (LCS), proved to be
an elegent theory and capable of generalizing well between different verbs; in the 90ies LCS
was implemented as system for representing semantics in early NLU und translation models \citep{palmer2010semantic}.
...
\cite{dowty1991thematic} in contrast, approached the proeblem of constructing
a framework for analysing core conceptual semantic structures from a different
...
However, different resources have been created, implementing some variaty of the various Semantic Roles frameworks.
...
been the target of most of the existing two million words of PropBank annota- tion. Nonetheless,
to fully capture event relations, annota- tions must recognize the potential for their expression
...
Following are soe exmample sentences from the PropBank frames\myfootnote{accessible through this
