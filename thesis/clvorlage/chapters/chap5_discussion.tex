...
One might ask if accuracy is the right metric to make this substantial claims
...
In other words, it is expected for GliBERT to show difficulties on the aforesaid datasets, which
...
Therefore, as for the data quality / label noise assessment, I conduct a human evaluation study
...
comprises often several sentences).}
...
\tab{tab:srl-abla}{Ablation on effect of PREDs and ARGs information isolated. Note that mostly only the combination of both information structures leads to a significant improvement over vanilla BERT embeddings ($-$SRL).}{
  \scalebox{0.9}{
...
By informing the model solely about where arguments are located in a sentence leads to a slightly better performance, but
...
Therefore, the crucial aspect of SRL information the GliBERT heads improve from lies
...
\end{srl}
\label{srl:ablation}
\captionof{srl}{Only predicate SRLs (\textbf{left}) and only argument SRLs (\textbf{right}) of \ref{srl:normal}. The experiments revealed
  that models already slightly improve by having access to these reduced SRL information structures in isolation, but only if the
  full, combined SRL information is provided, the performance increases reach their maximum (see table \ref{tab:srl-abla}).}
\endgroup
...
determined.\myfootnote{Only the significant results from table \ref{tab:gain-loss} were
taken into account for the computation of the overall gain.} For question answering datasets
...
the only significant result being a deterioration of 1.01\%.\myfootnote{If all results from
table \ref{tab:qa-gain-loss-token-merged} were taken into account, an average decrease of
0.07\% after adding SRLs over all experiments would be measured.}
...
coping approaches presented: Duplicating the first predicate-argument structure to
...
For merging the SRL embeddings with the BERT embeddings, also two modes were proposed: Merging BERT
subtokens back to token level by averaging the respective vectors, or duplicating the SRLs to match
up with the BERT subtokens. Over all datasets, and for question answering datasets in particular,
duplicating the SRLs and preserving the BERT subtoken embeddings clearly led to better results than
the latter approach: Out of total 56 experiments, 36 times the BERT subtokens preserving approach
performed better than the merging approach --- 61.11\% of it statistically significant. From the 20
times the merging approach was better, only 4 of them were statistically significant. Presumably,
this has to do with the inevitable information loss during the subtokens averaging process.
