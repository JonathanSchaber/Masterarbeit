...
Since the different
dataset types were tested with different heads, I group the them the following way: Results for classification
datasets (deISEAR, SCARE, PAWS-X, XNLI) are reported together, and the results for question answering datasets
(MLQA, XQuAD), are grouped together. Since the latter group consists of only two datasets
addressing the same task, and bearing in mind that there were only experiments
for one head in this group, the Span Prediction Head, the discussion analyzing these results
will not be as interesting and substantial as for the first group. For space reaons, and for the sake of straightforwardness,
only test set results are reported in the tables. However, model selection was always based on development set results (see
figure {fig:acc-loss} for visualizations of development and test resulst compared).


\fig{images/acc_loss-3_sets.pdf}{fig:acc-loss}{Accuracy and loss plots for three experiments
(\textcolor{std-blue}{training}, \textcolor{std-red}{development}, and \textcolor{std-green}{test} set):
% \myfootnote{\textbf{deISEAR} +SRL normal duplicate [CLS] head, \textbf{SCARE} SRL merge zeros GRU
% head, \textbf{XNLI} SRL merge zeros [CLS] head}
In the deISEAR plot, we see that early stopping
was triggered after the loss on the development set increased for four contiguous epochs ---
indicating an overfitting of the model on the training set. Both SCARE and XNLI experiments ran
until the maximum number of epochs were reached without triggering early stopping. However, the
SCARE accuracy and loss progressions show much more fluctuations and erratic behavior than the
XNLI ones, indicating the noisiness of the data and the randomness of the function that needs to
be learned by the model.

The vertical line marks the epoch with the highest development accuracy, the dashed horizontal
marks the corresponding test set value, and the drawn through the development value, respectively.
}{12}{Accuracy/Loss plots of three experiments}

\subsection{Controlling for Statistical Significance}
...
on the basis of measured perfmoance on a dataset.
...
is implementd.} In table \ref{tab:results} below, the test set ensemble results for
...
reoprted in the table. For example, we look at the [CLS] head on deISEAR $\alpha$ and ask us, which
...
for +SRL zeros and +SRL duplicate is calculated and controlled for statistical signifcance between
...
it significantly. Therfore, at least for the GerGLU classification datasets, splitting the SRLs
...
and was taken into account; sometimes both implementations permormed equally, which is
...
summarized in figure \ref{fig:fig:classification-gains}. Doing this, the image
clkarifies, allowing to stipulate that adding SRLs on GerGLUE classification tasks
leads to a measurable improvement. The total gains clearly outweigh the losses, and also
more statistically signifacnt. Regarding the SRL implementation, ther is no clearadvantage
...
As described in chapter \ref{chap:3_datasets}, the datasets comiled in GerGLUE are a quite
heterogenous assemble: Some datasets like deISEAR and, especially, SCARE comprise rather
informal, colloquial tests, while PAWS-X and XNLI consits of more technical, standardized text.
