...
development accuarcy (the global maximum of the red curve), and the horizontal dotted line
marks the correpsonding test accuracy. On the right the corresponding losses for the
same epoch are marked in the same manner.
...
\begin{tcolorbox}[
  ...
  To control for two models, or ensembles, if the differences between the control (-c) and test
  (-t) one are statistically significant, run the following command (make sure the \texttt{conda}
...
self training'' technique leading to these results is mentioned, but which is unfortunately not elaborated upon any more. For their
``regular'' SemBERT large implementation the reported gain is 1.9\% --- which would still stand
for a positive influence of SRL enriching which is not reproduced through my experiments.}
