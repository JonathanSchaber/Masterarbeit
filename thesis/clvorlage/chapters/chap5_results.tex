...
\tab{tab:tab:hyper-configs}{Stable hyperparameter configurations.}{
  \scalebox{0.9}{
    \begin{tabular}{ll}
    learning rate              & 2e-05                         \\
...
\tab{tab:alpha-beta}{Differences betwenn $\alpha$ and $\beta$ runs for each dataset.}{
  \scalebox{0.9}{
    \begin{tabular}{lll}
                               & $\alpha$                   & $\beta$                    \\ \hline
    deISEAR                    & maximum length: 100        & maximum length: 40         \\
    SCARE                      & maximum length: 100        & maximum length: 50         \\
    PAWS-X                     & Examples train set: 8,000  & Examples train set: 48,977 \\
    XNLI                       & Normal predefined splits   & Resplitted 70:15:15        \\
    MLQA                       & Normal predefined splits   & Resplitted 70:15:15        \\
...
\begin{figure}
  ...
  \caption[Heatmaps subtokenizd and merged]{Gain/Loss heatmaps from the gain/loss values in table \ref{tab:gain-loss}. \textbf{Left}: Subtokenized. \textbf{Right}: Merged.}
\end{figure}

In figure \ref{fig:heatmap-gains} the absolute gains and losses are plotted in two heatmaps:
In the heatmap for the subtokenized settings, the pattern seems to be that for the single
...
To show this, I conducted a relabelling of 20 random examples from XNLI and PAWS-X, each, by
2 humans.
