...
deepset only provides a BERT base model which has the following specifications
...
sentences. Research by \citeauthor{zhang2019semantics} suggests that fixing the number of predicate-argument
structures to three yields the best results; so I adopt this number. In other words, if a sentence has more
than three argument-predicate structures, I only care about the first three predicates identified (if
proceeding from left to right through the sequence), and disregard the others. However, if there are
fewer than three predicate-argument structures present, I compare two strategies:
The first solution (the left sentence in figure \ref{srl:zeros-duplicate})
...
asdasdasdasdasdasjdhgashjdgasdjhags jhasgd jashdg asjdga dasg jashdg ajshdg asjd gasjdhg ajsdhg
asdjh gasdj hagd jhasdg

asdah sashdg askjdh gsadkjahsdg askjdh ga sdkjahsgdajsdhgajdhgasd jhagsd akjshdg asjdkhga sdsgd
akjshdg askjdhga sdhgsd ajshdg
