...
from deepset\myfootnote{\url{https://deepset.ai/german-bert}} which is available in \texttt{pyTorch}
...
\tab{tab:bert-hyperconfigs}{German BERT hyperparameter configuration.}{
  \scalebox{0.9}{
    \begin{tabular}{ll}
      ...
      Maximum Sequence Length (BERT-Subtokens) & 512 \\
...
additional weight decay), which updates BERT's weight matrices.\myfootnote{After plugging all
the different modules together, GliBERT is one large model whose shared parameters all get
updated by the AdamW optimizer according to the loss, not just the BERT layers.}
...
for German, e.g. \cite{gerdes2001word, gross2015dependency}.} In other words, in the English
...
token's STTS tag\myfootnote{The Stuttgart-Tübingen-TagSet (STTS) \citep{schiller1999guidelines}
is the de facto standard for tagging POS in German with the vast majority of POS-annotated
corpora for German implementing STTS.}.
...
\fig{images/exampleParzu2.png}{fig:example-parzu}{
  Example ParZu dependency parse tree for a sentence with several verbal forms. Different
  information per word is displayed: The token surface form, the CPOSTAG, the POSTAG, the lemma
  of the word, morphological information, and the index. Algorithm \ref{alg:find-predicates}
  correctly identifies ``abgewiesen'' and ``beschrieben'' as predicates for which SRLs can be
  predicted. Visualization taken from \url{https://pub.cl.uzh.ch/demo/parzu/}.
...
for each other token in the sequence the BIO-encoded SRL.

\fig{images/num_predarg_structs.pdf}{fig:num-predarg-structs}{Number of predicate-argument structures in all datasets.
                                                              Due to its boilerplate template form, deISEAR shows a peculiar distribution:
                                                              Since its examples always begin with ``Ich [\textsubscript{PRED} fühlte] $X$, als ....'',
...
but several, especially in longer sentences --- or even none, particularly in colloquial, short
sentences. Research by \citeauthor{zhang2019semantics} suggests that limiting the number of
...
the first three predicates identified (when processing from left to right through the sequence),
...
\captionof{srl}{The two strategies for dealing with less than three predicates: \textbf{Left}:
                The open SRL slots get filled with the special SRL \emph{0} (``zero'', not the letter O).
...
Since the vanilla classification and question answering heads differ quite substantially
...
Mathematically, the $softmax$ function $softmax: \mathbb{R}^{n} \rightarrow [0,1]^{n}$ is
defined as: \[ softmax(x)_i = \frac{e^{x_i}}{\sum_{j=1}^{n} e^{x_j}} \] for $i = 1, \dotso ,
n$.} to it, and selecting the component, i.e. class, with the highest probability:
...
\fig{images/head-lhsa.png}{fig:ffnn-head}{Head with a one-layer
  ...
  the concatenated token sequence. In contrast to the vanilla BERT
