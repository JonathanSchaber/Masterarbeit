...
\begin{tcolorbox}[
  ...
  The slot-filling mode can be controlled via the \texttt{zeros} attribute in
...
\begin{tcolorbox}[
  ...
  The aligning mode can be controlled via the \texttt{merge\_subtokens} attribute
  in the \texttt{config.json} file: If it is set to \texttt{true}, then the BERT
  embeddings of subtokens get merged into one token emebdding, if it is set to
...
\fig{images/head-lhsa.png}{fig:lloa-head}{Head with a one-layer
...
\fig{images/head-gru.png}{fig:gru-head}{Head with a one-layer \textbf{F}eed \textbf{F}orward \textbf{N}eural
  \textbf{N}etwork on the concatenated last hidden states of the bi-directional GRU. Predictions are made
  on the concatenated last hidden states of the forward and backward layer.}{10}{GRU Head}

\citep{cho2014learning} presented the GRU architecture, which similarly to the LSTM
