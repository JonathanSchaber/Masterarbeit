...
\begin{examples}
	\item \label{itm:tok-sentence} Anstieg um 4° zwischen 1990-2010
\end{examples}
...
\tab{tab:tokenization}{Tokenization aligning difficulties displayed on example
sentence \ref{itm:tok-sentence}. Note that in this case both tokenization sequences would need duplication.
...
{\begin{tabularx}{\textwidth}{m{3.4cm}m{3.4cm}|m{3.4cm}m{3.4cm}}
  ...
  Anstieg        & Anstieg            & \customcolorbox{Anstieg}{white}  & \customcolorbox{Anstieg}{white}       \\
...
(1) Take the ParZu-tokenized and parsed sentence, apply the predicate-finding algorithm to
it. Then, (2) tokenize the sentence using the BERT tokenizer, identify the afore detected
predicates\myfootnote{Since predicates are normally verbal forms without any special characters
in them, a string comparison search suffices to achieve this.} and hand the BERT tokenized,
predicate marked token sequence to DAMESRL.
